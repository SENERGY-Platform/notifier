package mock

import (
	"context"
	"encoding/json"
	"errors"
	"fmt"
	"github.com/SENERGY-Platform/notifier/pkg/configuration"
	"github.com/gorilla/mux"
	"github.com/senergy-platform/vault-jwt-go/vault/vaultjwt"
	"log"
	"net/http"
	"net/http/httptest"
	"runtime/debug"
	"time"
)

func MockKeycloak(config *configuration.Config, ctx context.Context) (err error) {
	router, err := getKeycloakRouter()
	if err != nil {
		return err
	}
	server := httptest.NewServer(router)
	config.KeycloakUrl = server.URL
	go func() {
		<-ctx.Done()
		server.Close()
	}()
	return nil
}

func getKeycloakRouter() (router *mux.Router, err error) {
	defer func() {
		if r := recover(); r != nil && err == nil {
			log.Printf("%s: %s", r, debug.Stack())
			err = errors.New(fmt.Sprint("Recovered Error: ", r))
		}
	}()
	router = mux.NewRouter()

	router.HandleFunc("/auth/realms/master/protocol/openid-connect/token", func(writer http.ResponseWriter, request *http.Request) {
		writer.Header().Set("Content-Type", "application/json; charset=utf-8")
		err = json.NewEncoder(writer).Encode(vaultjwt.OpenidToken{
			AccessToken:      "eyJhbGciOiJSUzI1NiIsInR5cCIgOiAiSldUIiwia2lkIiA6ICIzaUtabW9aUHpsMmRtQnBJdS1vSkY4ZVVUZHh4OUFIckVOcG5CcHM5SjYwIn0..eOwKV7vwRrWr8GlfCPFSq5WwR_p-_rSJURXCV1K7ClBY5jqKQkCsRL2V4YhkP1uS6ECeSxF7NNOLmElVLeFyAkvgSNOUkiuIWQpMTakNKynyRfH0SrdnPSTwK2V1s1i4VjoYdyZWXKNjeT2tUUX9eCyI5qOf_Dzcai5FhGCSUeKpV0ScUj5lKrn56aamlW9IdmbFJ4VwpQg2Y843Vc0TqpjK9n_uKwuRcQd9jkKHkbwWQ-wyJEbFWXHjQ6LnM84H0CQ2fgBqPPfpQDKjGSUNaCS-jtBcbsBAWQSICwol95BuOAqVFMucx56Wm-OyQOuoQ1jaLt2t-Uxtr-C9wKJWHQ",
			ExpiresIn:        10 * float64(time.Hour),
			RefreshExpiresIn: 5 * float64(time.Hour),
			RefreshToken:     "",
			TokenType:        "",
			RequestTime:      time.Now(),
		})
		if err != nil {
			log.Println("ERROR: unable to encode response", err)
		}
	}).Methods(http.MethodPost)

	return
}
